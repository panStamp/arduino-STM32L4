...
class QUANTUM
{
  public:
    ...
    /**
     * Time in seconds when sleep mode is entered
     */
    uint32_t sleepTime;
     ...
     * @param sec seconds to sleep
     */
    inline void sleepSec(uint16_t sec)
    {
      uint32_t elapsed = 0;
      sleepTime = RTC.getY2kEpoch();
      ...
      while (elapsed < sec)
      {
        STM32.stop((sec - elapsed) * 1000);
        elapsed = RTC.getY2kEpoch() - sleepTime;
      }
